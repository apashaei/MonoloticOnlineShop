...
namespace WebSite.EndPoint.Models
{
    ...
    {
        private readonly IGetMenuItem getMenuItemService;
        public MenuItemTagHelper(IGetMenuItem getMenuItemService)
        {
        ...
        {
            var data = getMenuItemService.GetMenuItems();
            TagBuilder liMain = null;
            foreach (var item in data.Where(p => p.ParentCategoryId == 0))
            {
